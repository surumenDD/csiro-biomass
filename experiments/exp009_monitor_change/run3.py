...
def train_one_fold(
    ...
    ckpt_best_r2 = ModelCheckpoint(
        dirpath=str(output_dir),
        filename=f"model_fold{fold}-best-r2",
    ...
    trainer = pl.Trainer(
        ...
        devices=devices,
        logger=wandb_logger,
        callbacks=[ckpt_best, ckpt_best_r2, ckpt_last, lr_cb, early_stop_cb],
    ...
    best_path = ckpt_best_r2.best_model_path
...
def make_folds(
    ...
    # ログ確認用
    df["Sampling_Date_Month"] = df[group_col].astype(
        str).apply(lambda x: x.split("/")[1].strip())
    ...
    for fold, (_, val_idx) in enumerate(sgkf.split(X=X, y=y, groups=groups)):
        ...
        trn_months = sorted(int(m)
                            for m in trn_df["Sampling_Date_Month"].unique())
        val_months = sorted(int(m)
                            for m in val_df["Sampling_Date_Month"].unique())

        print(
            f"trn({trn_df.shape[0]}) -> val({val_df.shape[0]}): {trn_months} -> {val_months}")
...
def _normalize_key_cols(df: pd.DataFrame, key_cols: list[str]) -> pd.DataFrame:
    """
    mergeの事故（floatの微小な差や型の不一致）を防ぐための正規化
    """
    out = df.copy()
    for c in key_cols:
...
def attach_folds_from_csv_ver4(
    ...
    """
    外部CSV (folds_opt_ver5.csv) を読み込み、train_wideにfoldを付与する
    """
    ...
    if miss:
        raise ValueError(f"folds_csv missing columns: {miss}")

    # キーの正規化
    left = _normalize_key_cols(train_wide, key_cols)
    right = _normalize_key_cols(folds_df, key_cols)

    # 重複チェック（CSV側のキーが一意であることを確認）
    if right.duplicated(subset=key_cols).any():
        raise ValueError("folds_csv has duplicated keys based on META_COLS.")
    ...
    if merged[fold_col].isna().any():
        ...
        raise ValueError(
            f"Some rows could not be matched to folds_csv. Missing: {n_miss}")

    merged[fold_col] = merged[fold_col].astype(int)

    # 各Foldのサンプル数表示
    vc = merged[fold_col].value_counts().sort_index()
    print("=== fold sizes (rows) from CSV ===")
    for f, count in vc.items():
        print(f"fold={f}: n_rows={count}")
...
def main(cfg: Config) -> None:
    ...
    with trace("attach_folds"):
        # cfg.env.folds_dir 内の指定のCSVを読み込む
        folds_csv_path = Path(cfg.env.folds_dir) / "folds_opt_ver5.csv"
