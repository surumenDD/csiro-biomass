...
class ExpConfig:
    ...
    # 使うならデフォルトもこっちに
    scheduler_name: str = "plateau"   # もしくは yaml 側で上書き
...
def main(cfg: Config) -> None:
    ...
    # with trace("make_folds"):
    #     sgkf = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=cfg.exp.seed)
    #     fold_indices = np.zeros(len(train_df), dtype=int)
    #     for fold, (_, val_idx) in enumerate(
    #         sgkf.split(
    #             X=train_df["object_id"].values,
    #             y=train_df["sorting_date"].values,
    ...
    # train per fold
    # 以下は過去コンペでの実装例。
    # oof_pred_full = np.zeros((len(train_df),), dtype=np.float32)
    # fold_scores: dict[int, float] = {}
    # fold_to_ckpt: dict[int, str] = {}
    # for fold in cfg.exp.folds:
    #     with trace(f"train_fold_{fold}"):
    #         oof_pred_fold, fold_acc, best_ckpt = train_one_fold(
    #             fold=fold, train_df=train_df, images_root=photos_dir, cfg=cfg, output_dir=output_dir
    #         )
    #         oof_pred_full[train_df["fold"].values == fold] = oof_pred_fold
    #         fold_scores[fold] = fold_acc
    #         fold_to_ckpt[fold] = best_ckpt
    # ===== train per fold =====
    n = len(train_wide)
    ...
    # OOF scoring
    #  以下は過去コンペでの実装例。
    # oof_pred = oof_pred_full.astype(np.float32)
    # oof_pred_clip = np.clip(oof_pred, 0, 3)
    # y_true = train_df["target"].values.astype(float)
    # oof_rmse = float(np.sqrt(np.mean((oof_pred_clip - y_true) ** 2)))
    # LOGGER.info(f"OOF RMSE: {oof_rmse:.4f}")
    # # If you need to log to W&B, rely on the Lightning WandbLogger per-fold
