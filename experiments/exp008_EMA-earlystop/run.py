...
def make_folds(
    ...
    # ログ確認用
    df["Sampling_Date_Month"] = df[group_col].astype(str).apply(lambda x: x.split("/")[1].strip())
    ...
    for fold, (_, val_idx) in enumerate(sgkf.split(X=X, y=y, groups=groups)):
        ...
        trn_months = sorted(int(m) for m in trn_df["Sampling_Date_Month"].unique())
        val_months = sorted(int(m) for m in val_df["Sampling_Date_Month"].unique())

        print(f"trn({trn_df.shape[0]}) -> val({val_df.shape[0]}): {trn_months} -> {val_months}")
...
def main(cfg: Config) -> None:
    ...
    scores = np.array([fold_scores[f] for f in ordered_folds], dtype=np.float64)
    ...
    LOGGER.info("BEST-CV per fold: %s", {f: float(fold_scores[f]) for f in ordered_folds})

    print(f"[BEST-CV] mean={best_mean:.6f} std={best_std:.6f} folds={k}{suffix}")
    print("[BEST-CV] per fold:", {f: float(fold_scores[f]) for f in ordered_folds})

    # もし「n_folds 全部回しているときだけ平均を信用したい」なら警告を出す
    if expected is not None and k != int(expected):
        LOGGER.warning("BEST-CV mean is computed on partial folds: got %d, expected %d", k, int(expected))
