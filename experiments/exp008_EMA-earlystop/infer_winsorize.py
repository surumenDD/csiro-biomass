...
def predict_ensemble(
    ...
    for p in ckpt_paths:
        ...
        pred3 = np.concatenate(preds_all, axis=0).astype(np.float32)  # (N,3)
        pred_list.append(pred3)

    preds = np.stack(pred_list, axis=0).astype(np.float32)  # (K,N,3)

    # まず NaN が無いか確認（NaN があると winsorize の結果が壊れやすい）
    if np.isnan(preds).any():
        raise ValueError("NaN found in ensemble preds before winsorize.")

    # fold/ckpt 方向（axis=0）で winsorize してから平均
    preds_w = winsorize(preds, limits=(0.2, 0.2), axis=0)  # :contentReference[oaicite:7]{index=7}
    pred3_winmean = np.asarray(preds_w.mean(axis=0), dtype=np.float32)  # masked array 対策
    return pred3_winmean
...
def main():
    ...
    WEIGHTS_DIR = Path("/kaggle/input/exp008-ema-earlystop-001-dataset")        # 重みデータセット名に置換

    CKPTS = [
        WEIGHTS_DIR / "model_fold0-best-v1.ckpt",
